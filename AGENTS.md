...
10. 正常交付一律使用 `docker compose up --build -d`（research-world/），不裸跑本地实例；Pi 只允许在宿主机用于开发端到端验证，Docker 不包含也不支持 Pi；模型凭证只进入 Runtime 进程，Runtime 在其进程内解析部署提供的模型访问配置，browser/control、公开 API、Trace 与日志不携带密钥。
