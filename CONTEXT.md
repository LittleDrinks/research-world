...
**Research World 设置 facade**：Web、Research Kernel、Runtime 与 Penguin 的组合边界，转发模型设置操作并协调 Chat；不持久模型访问配置，不拥有 Penguin token。
...
**Penguin Harness Adapter（Docker/MVP 默认）**：Compose 管理固定 Penguin Server 进程与 readiness，Runtime Adapter 只封装 Penguin 原生 HTTP/SSE、Penguin Session 与 Task 映射；模型访问配置由 Penguin 持久化，凭证不进入 Agent、Session、Trace、control 或 Web。
**模型访问配置**：部署级设置，包含 provider、model id、baseurl 与 apikey；Research World 设置 facade 负责转发测试、保存、掩码读取、替换与清除，Penguin 是唯一持久所有者；不随 Project、Session、Agent、Run、Turn 或 Trace 复制。
