import { Bot, ChevronDown, ChevronRight, SquareArrowOutUpRight } from "lucide-react";
...
export function RunCard({ run, threadId }) {
  ...
  return <article className={`run-card ${open ? "open" : ""}`}>
    <div className="run-card-head-row">
      ...
      <TraceLink runId={run.id} threadId={threadId} /></div>
    {open && <div className="run-card-body">
