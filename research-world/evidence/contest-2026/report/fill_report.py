...
INTRO = (
    "科学假设生成是连接开放科学问题与可检验研究计划的关键环节，而大模型直接作答难以区分已有事实与模型推断，"
    "也难以保证可检验性与引用可回溯。本作品构建作者、评审、修订相互制衡的研究工作流：独立 Qwen 作者会话校正题干前提、"
    "识别缺口并核验证据，生成三个机制不同的候选方向，经比较筛选形成标注计划与执行边界的研究计划；"
    "独立评审以六维量表与来源门逐条核验，评审发现回流驱动多轮修订。官方 125 题全量运行均形成候选结论"
    "（完成 8、部分 117、失败 0，2592 次调用）；五个深度案例经修订均达 12/12，旗舰案例由 9/12 修至 12/12。"
    "全部账本与哈希可回读，候选结论不等同于已验证发现。"
)

QWEN_NOTE = (
    "本作品以 Qwen 系列模型为唯一作者基座，经阿里云百炼 OpenAI 兼容端点调用。作者会话采用 qwen3-max，"
    "分层拼接系统协议、规范原题、来源记录、历史评审发现与计划执行边界，完成问题理解、证据检索、方向生成、比较筛选与研究计划设计；"
    "为避免自评偏置，评审与回执环节由其他模型交叉执行，评审侧仅可见固定量表与被审版本，引用一律经独立检索复核。"
    "当评审结论为退回修订时，评审发现逐条回流触发新一轮修订，形成多轮闭环；模型调用、token 消耗与输出哈希全部入账本，"
    "支持事后审计与复现核验。"
)
