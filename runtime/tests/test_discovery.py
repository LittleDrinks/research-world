...
def test_discovers_workspace_tool_without_exposing_secrets(tmp_path, monkeypatch):
    ...
    _write_search_definition(tmp_path)
    public = discover_adapters(tmp_path)["search"].inspect()
    _assert_private_tool_public(public, tmp_path)


def _write_search_definition(path):
    server = {
        "type": "http", "url": "https://mcp.test",
        "headers": {"Authorization": "Bearer ${SEARCH_TOKEN}"},
    }
    path.joinpath(".mcp.json").write_text(json.dumps({"mcpServers": {"search": server}}))


def _assert_private_tool_public(public, path):
...
def _assert_catalog_shape(value):
    ...
    assert all(set(item) == {"id", "name", "description", "source", "status"} for item in tools.values())
