...
|---|---|---|---|---|---|
| P1 | 盖章报名表第 1、2 页截图；系统报名作品名、最终作品名、赛道方向、300 字作品简介、300 字 Qwen/AI 说明、可选宣传链接；具体问题、核心方法、代表结果 1/2（对象、口径、结果）、主要局限；API/前端、代表案例、报告、源码、Qwen 调用凭证/截图、125 题输出、可选 10 分钟视频 | 汇总经核验的作品身份、方法、两项科学结果与限制；从 Project/报告投影生成可追溯摘要；报名所需个人信息按模板提交，非必需个人信息脱敏，受限数据、密钥不交付 | 原模板 `docs/赛道一-方向1A-科学假设生成与研究计划设计-提交要求及模板.docx`；代表案例 `project.json`；`research-world/evidence/contest-2026/deep-cases.md` | 五题 canonical 输入与深度结果存在；报名字段、两项代表结果的提交投影、125 题全量输出和交付入口尚未形成 | 报名信息由团队提供；#36、#44、#245、#112 |
| P2 | 现有方法的具体不足、实际解决范围；已实现的科学问题理解、证据组织、候选生成、比较筛选、研究计划、反馈修订；总体思路图；主要输出；125 题实际测试；与直接大模型问答的差异 | 形成“问题→知识缺口→证据→候选→筛选→计划→反馈”闭环并逐项证明已实现功能、实际输出、全量结果和同条件直接问答差异 | `docs/adr/0037-research-graph-and-dynamic-workflow.md`；`research-world/evidence/contest-2026/deep-cases.md`；五题各自 `run.md` | 五个案例已有 V1、独立评审、最终版、来源、计划和终态；q049 保留直接回答对照。125 题全量输出仍由 #245 执行，总体思路图未形成 | #45、#47、#245 |
| P3 | 对事实/文献解释/模型推断、知识缺口转候选、可检验/证伪、反对证据/替代解释、候选非结论五项分别填写实际做法与理由；说明识别缺口的信息、约束假设的证据及支持/削弱/否定结果 | Claim 与 Source/Experiment/Artifact 的证据闭包、Direction、supports/refutes 关系和独立审核；展示代表案例中每项原则的实际输入、判据和 verdict | `CONTEXT.md`；`docs/adr/0037-research-graph-and-dynamic-workflow.md`；五题 final/review | 五题已实跑来源约束、知识缺口、三条 Direction、替代解释、候选边界和独立 verdict；平台 Research Graph 导入不在本阶段 | #245 |
| P4 | 每条实际资料的名称、获得/使用方式、对假设的作用、局限；来源可用性判断；证据不足/冲突处理；引用与证据对应；只列实际使用内容 | 获取全文、核验 authors/year/identifier/license/hash 并归档 Source；Claim 绑定来源与 Artifact，冲突保留 supports/refutes 关系 | `docs/submission-reference-manifest.json`；五题 final/review/receipt | 五题当前来源已在 frontmatter 固定并经独立 review 抽查；错配 DOI、作者、题名、日期和标准语义的被拒历史均保留。平台 Source/Claim 导入不在本阶段 | #245 |
| P5 | 每项评价内容、具体方法、对象/样本、结果如何影响后续；评价重点与理由；程序/模型/团队/专家主体；V1 与最终版口径是否一致及变化原因；继续、停止、转人工条件 | 从 Trajectory、Artifact 和版本结果计算过程指标，冻结样本与 rubric；评价结果触发修订、停止或人工判断，报告口径变化 | `benchmarks/README.md`、`docs/benchmarks/design.md`；`research-world/evidence/contest-2026/deep-cases.md`；五题 `run.md` | 五题已按同一 rubric 保存 V1、逐轮评审、修订、模型/token 代价及最终处理；#56、#57 组件 benchmark 单列，不冒充案例成绩 | #47、#245 |
...
| P7 | 实际 Qwen 模型/调用方式、具体任务、单次上下文、结构化输出/格式约束、检索/代码/Tool 协作；真实上下文结构图含问题、已有/反对证据、关键约束、历史结果、反馈；抑制无依据生成措施、更新/多次调用触发、实际变化 | AgentSpec 快照模型与 Tool；独立 Session、来源引用和独立审核；Trajectory 证明真实上下文、调用次数、触发条件和前后结果 | `docs/adr/0022-agent-spec.md`、`docs/adr/0026-agent-runtime.md`；五题 `run.md` 与 Artifact/review/receipt | 五题已记录实际 Qwen/Codex 模型、独立 Session、调用、token、检索、修订触发和失败；提交版真实上下文结构图仍未形成 | #44、#245 |
| P8 | 识别对象/范围、条件/关键变量、已有认识/争议/未知、可处理知识缺口四步的实际做法、中间结果、对假设生成的用途；一题真实中间结果示例 | 主 Agent 把 Question 解析为约束与可引用知识缺口，并将结果交给 Direction 生成 | 代表案例 Project 输入；五题 final Artifact | 五题 final 已记录对象、范围、变量、已有认识、争议、未知与知识缺口，并把缺口用于三条 Direction；提交投影仍待生成 | #245 |
| P9 | 从缺口形成解释、多个可区分候选、纳入支持/反对证据、控制重复/空泛/不可检验、统一表达五项实际做法及解决的问题；每条候选含核心陈述、依据、正反证据、预测、替代解释、不确定性 | 主 Agent 生成三个不同 Direction，检索与独立审核控制重复并保留完整比较；给出真实候选数量和内容 | `docs/adr/0037-research-graph-and-dynamic-workflow.md`；五题 final Artifact 与 review | 五题均形成三个机制或路线可区分的 Direction，并保存依据、限制、预测、替代解释、不确定性和评审结果 | #245 |
| P10 | 相关性、证据一致性、冲突/错引、可检验/证伪、候选重复、是否进入计划六项的判断与处理结果；说明保留、合并、降级、淘汰规则；给出一组选/不选实例 | 独立 review 产生逐项 verdict，主 Agent 决定保留、合并、降级或淘汰 Direction，并保留对比候选及理由 | `docs/adr/0037-research-graph-and-dynamic-workflow.md`；五题 review 与 `run.md` | 五题独立 review 已记录 rubric、来源错引、科学缺陷、选路和返工结果；被拒版本与最终通过版本均保留 | #245 |
| P11 | 待验证预测、所需数据/资料/实验条件、研究步骤/分析方法、各结果支持/反对对象、停止/回退/补证条件；具体/可复核检查；已具备执行条件与建议项；禁止“进一步研究”空话 | 研究计划明确 planned 与 executed；实际执行保存输入、方法、输出、退出状态、数据或文件哈希和限制，主 Agent 按证据决定继续、修订、停止或转人工 | `docs/adr/0020-graph-and-evidence.md`、`docs/adr/0037-research-graph-and-dynamic-workflow.md`；五题 final 与 `run.md` | 五题均有数据或材料、方法、对照、判据、产物、资源伦理、停止条件和 planned/executed 边界；仅 q049 Peters 限定计算为 executed，其余不伪造执行 | #245 |
| P12 | 真实全流程图及反馈返回点；至少三项实际反馈/失败的情况、发现、处理、后续影响；自动反馈、人工调整、保留的前后变化 | 动态 Workflow 串联接题、候选、计划、执行、review、revision；V1、评审、最终版和失败 Trajectory 区分自动与人工反馈 | `research-world/evidence/contest-2026/deep-cases.md`；五题版本、review 与 `run.md` | 五题完整保留 V1→独立评审→修订→最终链，覆盖错引、数值错、标准越界、因果过度和上游失败；提交版流程图未形成 | #45、#245 |
| P13 | 125 题单题运行方式、共同输出、总体评价、证据不足/失败/人工判断保留；案例领域/特点、选择理由、展示能力、不能代表的题目/条件 | 对 125 个 canonical Project 使用同一任务协议和评价并保留全部终态；按研究形态选择案例且声明外推边界 | `research-world/evidence/contest-2026/submission-evidence.md`；`research-world/evidence/contest-2026/all/index.md`；`research-world/evidence/contest-2026/all/run.md` | Q001-Q125 已用独立作者 Session 形成候选并经五组独立审计；统一方法、案例选择和不可代表边界已形成提交投影 | — |
| P14 | 案例问题原文、对象/变量、已有认识/主要证据、知识缺口、不确定性/争议；第一轮证据/约束、生成目标、模型/方法设置、评价口径 | 将 canonical Question、Source 和约束投影为 V1 输入快照，保存模型、方法和 rubric | `research-world/evidence/contest-2026/submission-evidence.md`；`research-world/projects/q049/project.json`；`research-world/evidence/contest-2026/q049/v1.md` | q049 原题、对象、约束、三方向、实际模型与统一 rubric 已形成提交投影 | — |
| P15 | H-01/H-02/H-03 的依据、反证/替代、预测、第一轮处理结果；三步计划内容及每步可支持/反对/区分对象；保留重复、证据不足、不可检验和不具体的原始结果 | V1 保留全部 Direction、review verdict 和计划，不以人工精选覆盖原结果 | `research-world/evidence/contest-2026/submission-evidence.md`；`research-world/evidence/contest-2026/q049/v1.md`、`review-v1.md` | q049 V1 三方向、取舍、planned 计划、低分、错引与错误判据均保留 | — |
| P16 | 至少三项第一轮具体问题、判断依据、科学结论/计划影响、第二轮实际调整；触发证据/评价/人工意见；增删改、不变项及理由、预期改善 | 独立 review 绑定问题与修改要求，Revision Session 引用触发证据并保留未改项 | `research-world/evidence/contest-2026/submission-evidence.md`；`research-world/evidence/contest-2026/q049/review-v1.md`；后续版本与 review | q049 错配 DOI、错误 arXiv、反向转述、22 数量级错误与无效判据均有修订闭环 | — |
| P17 | V1 与最终版的证据、Direction、筛选、计划、不确定性/边界及变化原因/结果；改善、未改善/新代价、停止或继续理由 | 以同一 rubric 比较有次序的 V1 和最终版，Trajectory/Artifact/Session lineage 证明变化；不新增 Round 领域对象 | `research-world/evidence/contest-2026/submission-evidence.md`；q049 `v1.md`、`review-v1.md`、`v8.md`、`review-v8.md`、`receipt-v10.md`、`run.md` | q049 V1→独立评审→最终版的改善、代价、残余项、停止理由和唯一 executed 计算已形成提交投影 | — |
| P18 | 至少一种同条件对照/消融：对象、相同条件、评价方法、本作品结果、对照结果、结论/代价；分别说明科学逻辑、技术方法、结果改善及未改善/成本 | 冻结输入、模型/预算与评价口径执行对照，保存两侧 Trajectory/Artifact 和确定性指标 | `research-world/evidence/contest-2026/submission-evidence.md`；`research-world/evidence/contest-2026/q049/run.md`；`benchmarks/README.md` | q049 的实算近似与长度近似直接回答均披露不匹配项；外部 benchmark 仅保留方法/组件口径 | 无严格双匹配对照；不再重采样 |
| P19 | 125 题完整形成、部分/证据不足、无法处理/需人工、重复运行/稳健性的数量/比例、判断口径、表现/原因；至少三类失败/边界的表现、原因、当前能力；可选外部泛化来源/方法/结果；候选与计划仍需研究者审查 | 汇总全部 Project 状态并保留失败；复跑计算稳健性；披露输入串位、适用边界和人工审查，不把结构合法当语义正确 | `research-world/evidence/contest-2026/submission-evidence.md`；`research-world/evidence/contest-2026/all/index.md`、`all/run.md`；`deep-cases.md` | 全量轻量终态、Session/token 与 partial 口径已汇总；深度案例终态单列，候选与计划不表述为发现 | 未做同题重复运行，不能报告跨采样稳健性 |
| P20 | 源码/环境/依赖/运行方法；Qwen 模型/调用及不含密钥的凭证或截图；125 题逐题输出；案例 V1、评审、最终版、必要日志；测试 API/示例请求/交互前端；可选视频；≤20 页、事实一致、125 题不省略、案例不代替总体、入口可核验、候选非结论、敏感信息妥善处理自检 | 形成自包含交付包与可复现入口；逐项校验文件、服务、调用证据、输出、日志、API/前端和隐私；报名所需个人信息按 P1 模板交付，非必需个人信息脱敏，受限数据/密钥排除 | `research-world/evidence/contest-2026/submission-evidence.md`；仓库源码；`research-world/README.md`；`research-world/compose.yaml` | 源码、Compose、Qwen 运行账本、125 逐题输出、旗舰版本链、回执与哈希可回读；当前 checkout 缺少被忽略的 `.env`，未把本地端口声明当成公开服务可用证明 | 人工补 `.env` 与 Compose/健康检查、公开 API/示例响应、前端实测、脱敏 Qwen 凭证截图、raw JSONL 副本、报名排版与可选视频 |
