...
| P14 | 案例问题原文、对象/变量、已有认识/主要证据、知识缺口、不确定性/争议；第一轮证据/约束、生成目标、模型/方法设置、评价口径 | 将 canonical Question、Source 和约束投影为 V1 输入快照，保存模型、方法和 rubric | 代表案例 `project.json`；五题 `v1.md`、review 与 `run.md` | 五题已保存 canonical 问题、第一轮证据与约束、三方向、计划、实际模型/Session/token 和统一 rubric；来源逐项可回读 | #245 |
| P15 | H-01/H-02/H-03 的依据、反证/替代、预测、第一轮处理结果；三步计划内容及每步可支持/反对/区分对象；保留重复、证据不足、不可检验和不具体的原始结果 | V1 保留全部 Direction、review verdict 和计划，不以人工精选覆盖原结果 | 五题 `v1.md` 与首轮 review | 五题第一轮 Direction、依据、限制、预测、计划和缺陷均按原始字节保留；低分、错引与不可用判据没有被最终版覆盖 | #245 |
| P16 | 至少三项第一轮具体问题、判断依据、科学结论/计划影响、第二轮实际调整；触发证据/评价/人工意见；增删改、不变项及理由、预期改善 | 独立 review 绑定问题与修改要求，Revision Session 引用触发证据并保留未改项 | 五题 review、后续版本与 `run.md` Session lineage | 五题保存逐项 finding、来源核验、修订触发与前后结果；典型问题包括 22 数量级错误、错配 DOI、样本量错用、观察性因果过度与 EN 13432 语义越界 | #245 |
| P17 | V1 与最终版的证据、Direction、筛选、计划、不确定性/边界及变化原因/结果；改善、未改善/新代价、停止或继续理由 | 以同一 rubric 比较有次序的 V1 和最终版，Trajectory/Artifact/Session lineage 证明变化；不新增 Round 领域对象 | `research-world/evidence/contest-2026/deep-cases.md`；五题 V1、final、review 与 `run.md` | 五题均可按同一 rubric 比较 V1/最终版，保存来源变化、缺陷修复、新增 token 代价、未解决项与 completed/waiting_human 理由 | #245 |
| P18 | 至少一种同条件对照/消融：对象、相同条件、评价方法、本作品结果、对照结果、结论/代价；分别说明科学逻辑、技术方法、结果改善及未改善/成本 | 冻结输入、模型/预算与评价口径执行对照，保存两侧 Trajectory/Artifact 和确定性指标 | `research-world/evidence/contest-2026/q049/run.md`；`research-world/evidence/contest-2026/deep-cases.md`；`benchmarks/README.md` | q049 已保存同题同模型直接回答与 Workflow V1 的多次尝试、实际调用/token、rubric、来源和计划差异；最终采用实际资源近似的对照由独立 reviewer 裁决 | #47、#245 |
