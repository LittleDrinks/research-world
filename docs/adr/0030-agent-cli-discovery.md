...
每个内置 Runtime 候选始终进入 inventory；缺失、无效版本或 probe 超时只改变该候选的 `status` 与 `reason`。本阶段 inventory 只包含 Codex；Runtime、Endpoint 与 Model 是 AgentSpec 的独立引用，Catalog 不由 Runtime 合成、保留或推断 Endpoint id。Launch 对 Codex 以 Runtime readiness、已声明 Endpoint/Model 可用性和显式兼容性作为执行条件；缺失或不兼容直接失败，不 fallback。
...
## Probe
Runtime 对每个内置候选执行固定 argv allowlist，不经过 shell。定位、路径解析、版本与认证 probe 分进程运行；单步最多 2 秒、单候选累计最多 5 秒，stdout 与 stderr 各截断到 16 KiB，解析后丢弃。Codex candidate 在 snapshot 后以同一 deadline 覆盖 version、login 与进程组/reader 清理；单步超时或剩余预算耗尽均终止进程组并返回不含私有信息的 `probe_timeout`。一个候选失败不改变其他结果。
