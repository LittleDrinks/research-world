...
Graph CLI 的直接变更没有准入语义，主 Agent 的动态选择也不能预设审核次数、后续方向或终止路径。因此 `pending`、`admitted`、`ghost`、强制双审、Direction 状态机、Kernel Auto 和固定 Pipeline stage 均不存在。交付报告从指定 Fact Graph 快照及两个有序 Pipeline run 切点生成；V1/V2 不是研究流程。
## 边界
