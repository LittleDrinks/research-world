...
Graph CLI 的直接变更没有准入语义；将审核次数或 Direction 状态固化为图谱字段，会把主 Agent 的方法选择错误地变成存储约束。Kernel Auto 和固定 stage 又会重复主 Agent 的编排职责，并预设后续方向与终止路径。因此 `pending`、`admitted`、`ghost`、强制双审、Direction 状态机、Kernel Auto 和固定 Pipeline stage 均不存在。交付报告从指定 Fact Graph 快照及两个有序 Pipeline run 切点生成；V1/V2 不是研究流程。
## 边界
