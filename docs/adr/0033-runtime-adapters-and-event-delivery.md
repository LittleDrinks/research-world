...
Run 可持久化一个由所属 Adapter 提供的当前 identifier-only JSON 原生身份，形状固定为 `{"session_id": 非空字符串}`。每个 TurnRequest 只向该 Adapter 提供已校验的原生身份和绑定回调；首次绑定将身份与 Adapter 归属原子写入 Run，相同值幂等，冲突、未知字段、嵌套对象、数组、scalar 或凭证样式字段 fail closed。绑定载荷只在所属 Adapter 的 TurnRequest 回调 provenance 中流转；Runtime 不以任意字段或标量相等推断秘密，也不改写 `turn_start.input` 或普通 Adapter 事实。恢复先校验 Run、身份 envelope、当前身份形状与 Adapter 归属，再把身份交给 Adapter；未知 Adapter、坏结构或归属不一致时不调用 Adapter。绑定载荷的字段和值不进入 Agent 快照、Trace、result、error、log、events、Run public view、Kernel 或浏览器；普通事实即使恰好含有相同字符串也保持原样。任一 Turn 绑定拒绝后锁定为 error，后续绑定不再写入或清除该拒绝，未绑定 Adapter 的事件按各自 Turn 的产生顺序即时持久化和发布；旧 Run store 格式直接拒绝，不写 migration、backfill 或 fallback。
