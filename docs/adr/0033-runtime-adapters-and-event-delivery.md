...
Trace 是按 Turn 追加的 Runtime 执行事实。Runtime 先持久化事件再发布，事件序号用于订阅和重连；取消、错误与终态只改变目标 Turn。Cancel 只取消一个 Turn，重连只读取既有事件。Delegate 必须显式验证 `parent_turn_id` 属于指定的父 Run；该标识只属于 Delegation 关联，Child Run 仍只保存 `parent_run_id` 且 `session_id` 为 `None`。Child Turn 终态以包含 Child Run、Child Turn、状态和结果的 `child_result` 事件追加到该父 Turn Trace，不推断最近 Turn，不写入用户 Session。普通 parent 终态必须等全部已启动且关联该 `parent_turn_id` 的 Child Turn 将 `child_result` 写入父 Trace 后，才追加 parent `turn_end`；parent Adapter 以 `completed`、`limit` 或普通 `error` 结束后进入等待阶段，期间不接收新的 Delegate 或 Child Submit。显式 Cancel 或取消清理失败的 force 终态立即关闭 parent，绝不取消其他 Child Turn；之后到达的 Child 结果不写入已终态 parent Trace。
...
transport/session.py 是 Kernel 与 Runtime 之间的无状态协调层，不是第三深模块；顺序是持久化用户消息 -> Runtime Submit -> 将主 Agent 终态回答投影到同一消息。HTTP 的 Session 读取和发消息由 transport/session.py 负责，Runtime Turn 订阅、取消和 Run 查询由 transport/runtime.py 负责，Kernel Project、Artifact、图谱和 LocalMap 由 transport/kernel.py 负责；提交、订阅、取消和 Session 读取保持为分离路径。当前 HTTP 路径为 `GET/POST /api/v1/projects/{project_id}/sessions`、`GET /api/v1/projects/{project_id}/sessions/{session_id}`、`POST /api/v1/projects/{project_id}/sessions/{session_id}/messages`、`GET /api/v1/turns/{turn_id}/events?after_seq=...` 和 `POST /api/v1/turns/{turn_id}/cancel`；消息提交请求携带已启动主 Agent Run 的 `run_id`、稳定的 `message_id`（可省略时由 Kernel 生成）和 `content`，提交只返回 JSON。transport 通过装配注入 Runtime，不保存 Session 到 Run 的映射或其他业务状态；全局旧应用的 Runtime 装配留给架构切换。
