...
## 取代范围
上述取代只覆盖与新所有权和事件边界冲突的旧决定：ADR 0026 中 Runtime 拥有 Session、Kernel 保存 Thread 指针以及一次会话式 prompt/inspect 的范围；ADR 0027 中 Kernel 直接解释 Pipeline run 的范围；ADR 0032 中固定 Pipeline、Stage、Auto 执行入口及其 Runtime 绑定的范围；ADR 0037 中以 Thread、Trajectory 或 Workflow 驱动执行会话的范围。各 ADR 未与 Run、Turn、Trace、Adapter、Session 所有权冲突的研究图谱、报告和发现细节仍是历史记录，不重写。
