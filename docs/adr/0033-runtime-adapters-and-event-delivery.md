...
Run 可持久化一个由所属 Adapter 提供的当前 identifier-only JSON 原生身份，形状固定为 `{"session_id": 非空字符串}`。每个 TurnRequest 只向该 Adapter 提供已校验的原生身份和绑定回调；首次绑定将身份与 Adapter 归属原子写入 Run，相同值幂等，冲突、未知字段、嵌套对象、数组、scalar 或凭证样式字段 fail closed。恢复先校验 Run、身份 envelope、当前身份形状与 Adapter 归属，再把身份交给 Adapter；未知 Adapter、坏结构或归属不一致时不调用 Adapter。原生身份不进入 Agent 快照、Trace、Run public view、Kernel 或浏览器；旧 Run store 格式直接拒绝，不写 migration、backfill 或 fallback。
