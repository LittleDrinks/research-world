...
每个活跃 Turn 拥有独立的 Adapter 执行句柄；默认不共享可变的原生 harness 会话，只有 Adapter 明确支持 multiple writers 时才允许共享。每个 Turn 创建时冻结其 Run 先前的终态上下文，也不把其他活跃 Turn 的输入或生成内容加入快照。并发 Turn 的完成顺序不改变回答与起始用户消息的配对关系。
