...
supersedes:
  ...
  - "0038: Web model settings management operations, Penguin ownership of model access config, and the credential-delivery rules over password-input apikey handling and the repository-root environment exclusion"
...
Pi Adapter、Delegate、Child Run 与 Tool 不参与 Provider Web Chat 模型访问路径；刷新恢复沿用既有重复 Submit 幂等语义；WebUI API-key 设置不是模型访问配置的解析来源。
