...
`resolve_admission` 是 pending 节点的唯一显式完成命令，只接收 node id、decision、reason 与 rebuttal；Kernel 构造 Verdict 并一次提交 Life state、rebuttal、claims 与事件。HTTP PATCH、CLI、Pipeline 和调用方构造的 Verdict 均不能改 Life state。Pipeline 的机制审核、证据审核与双审升级是 Kernel 内部策略，不扩大外部接口。
## 运行
...
Artifact 在 Project 内按 SHA-256 寻址，读取、关联与复跑校验都携带 project id。Connector 调用结果先登记为 Artifact，再通过显式 observation Submission 进入 Admission；Connector 成功不等于研究事实准入。
