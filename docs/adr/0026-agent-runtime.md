...
## Session 与 Trace
Trace 是 Session 的唯一模型事实源，不建消息数据库。凡进入模型的内容都写入 Trace；凭证、授权头、私有环境变量、workspace、Codex home 与 continuation binding 永不写入。inspect 保留 token budget 和 input/output token 用量。事件为 `session_meta`、`turn_start`、`model_request`、`model_response`、`tool_call`、`tool_result`、`child_session`、`turn_end`、`error`。
