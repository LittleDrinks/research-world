...
Codex 启动的 `OSError` 在公开边界重写为独立 `TraceError`；公开错误对象及其可遍历的 `__cause__`、`__context__` 链均不保留原始异常、fd、路径或其他私有执行细节。`close` 与启动交错时，关闭等待中的每个返回 child 都会停止；`close` 返回即清空 `_starts`、`_processes`、`_stops`、`_stopped`，不得依赖尚未收敛的 `generate` finally。
## Tool Runtime
