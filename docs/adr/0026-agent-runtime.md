...
Codex 只接受已声明 `openai-compatible` adapter 的逻辑 Endpoint 和归属 chat model；它们不提供 CLI transport 或凭证。公开 prompt result、`model_response` 和 `turn_end` usage 仅含 `input_tokens`、`cached_input_tokens`、`cache_write_input_tokens`、`output_tokens`、`reasoning_output_tokens`。当前或本次结果的 continuation 值从所有原始、公开 Trace 字符串和嵌套 payload 中脱敏；只在完成或达上限后写入 `0600` 私有 state。缺失、无模型或不兼容的显式声明直接失败，不生成、保留、推断 Endpoint，也不 fallback。
## Tool Runtime
