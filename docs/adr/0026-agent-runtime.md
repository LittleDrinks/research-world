...
## 内部所有权
`endpoints` 识别 OpenAI 兼容服务与 Codex CLI，并在同模型 Endpoint 间故障切换；`skills` 解析 `SKILL.md`；`connectors` 注册或识别 MCP server 并执行所选能力；`tools` 执行文件与渐进披露工具；`trace` 写入并重放 Session；`acp` 只翻译外部协议。外部调用方不感知这些目录。
