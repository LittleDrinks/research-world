...
Runtime adapter compatibility 是非空显式 adapter 集合；空集不兼容任何 Endpoint，Endpoint configuration 缺失或空 adapter 无效，EndpointPool 不提供默认 Endpoint。embedding 只在可用且有 provider 的 Endpoint 调用，否则返回 controlled unavailable。Codex 在 discovery 时解析并冻结实际可执行文件；probe、launch、resume 与 identity recheck 均针对同一解析目标。Codex child PATH 只含受信任系统目录和镜像安装目录，足以解析 pinned `env node` entrypoint，不继承宿主 PATH。
collector 在公开边界前从已知 parent、`thread.started.thread_id` 及官方 `collab_tool_call.sender_thread_id`、`receiver_thread_ids`、`agents_states` 的 key/value 学习全部 continuation id；每个学到的 id 都在字段值、嵌套 key 和文本中脱敏，碰撞键保留全部脱敏值。raw continuation 只走私有 transport 到受限 state，ModelResult、emit、message、Trace 和 inspect 都不携带它。
## Tool Runtime
