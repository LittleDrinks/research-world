...
- running run 以 30 秒租约认领，worker 每 5 秒续租；Control 只持久化人工信号，Worker 独占 Pipeline 推进；stage 按节点、审查、计划写检查点，runtime session 与实验 execution 用稳定操作 ID 恢复；失败 run 创建的 pending 节点转 ghost。
...
- Q49、Q89、Q21 已从正式 UI 完成项目创建、节点钉入、真实对话、brainstorm、轨迹返回、地图与科研日志；Q49 的 Codex `high` 规划经 3 次人工确认执行解析重叠、漂移映射与 6 组敏感性复现，实验与反思均通过双审，6 个 runtime session 各 1 turn；Q21 在 review 阶段强杀 worker 后恢复同一 run，9 个 runtime session 全部可读，4 个方向入图且无重复；Q21 的跨容器共享 `/tmp` 计划经人工驳回后暂停 run、ghost experiment，不改变 direction 且不产生证据边；Q89 research 负路径以 3 次容器执行复现量纲错误，双审拒绝实验并写入 refutes 边后暂停谱系。
...
- 深度演示题：Q49 正向 research pipeline 已完成；补完 Q89 Matbench 正向链路，再做 Q21 SciFact、Q112 湿实验返回、Q98 Sleep-EDF+手表返回；备选 Q110/Q32。
