...
def main():
    ...
    assert len(entries) == 97 and len({item["id"] for item in entries}) == 97
    assert sum(item["origin"] == "docs" for item in entries) == 84
    assert sum(item["status"] == "ready" for item in entries) == 94
    assert sum(item["status"] == "failed" for item in entries) == 3
    assert len(payload["unresolved_citations"]) == 17
    assert len(payload["excluded_sources"]) == 86
    for item in entries:
        check_entry(item)
    check_issue139(entries)
    check_exception(payload)
    check_generated_files()
    print(json.dumps({"errors": 0, "entries": 97, "ready": 94, "failed": 3, "pdf_markdown_pairs": 81, "issue139": 13, "unresolved": 17, "excluded": 86}, ensure_ascii=False))
